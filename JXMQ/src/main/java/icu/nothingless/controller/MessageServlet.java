...
public class MessageServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("/send".equals(path)) {
            ...
            resp.getWriter().write(JsonUtil.toJson(msg != null ? R.success(msg) : R.error("发送失败")));
            
        } else if ("/read".equals(path)) {
            // 标记已读
            Long friendId = Long.parseLong(req.getParameter("friendId"));
            messageService.markAsRead(userId, friendId);
            resp.getWriter().write(JsonUtil.toJson(R.success(null)));
            ...
            resp.getWriter().write(JsonUtil.toJson(success ? R.success(null) : R.error("撤回失败，超过2分钟")));
        }
    }
}