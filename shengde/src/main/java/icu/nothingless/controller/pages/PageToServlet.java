...
public class PageToServlet extends HttpServlet {
    ...
    static {
        ALLOWED_PAGES = loadAllowedPages();
        logger.info("PageServlet initialized with {} allowed pages", ALLOWED_PAGES.size());
    }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
        ...
        if (!ALLOWED_PAGES.contains(pageName)) {
            logger.warn("Page not in whitelist: {}", pageName);
        ...
        try {
            ...
            logger.error("Error rendering page: {}", pageName, e);
     ...
     * 从服务加载允许的页面列表
     */
    private static Set<String> loadAllowedPages() {
        try {
            ...
            logger.error("Failed to load allowed pages from service", e);
        }
        
        // 降级：返回默认页面集合
        return Set.of("dashboard", "profile", "settings");
    }
}